...
int8_t parseDHCPMSG(void)//20180625
{
  ...
  if((len = getSn_RX_RSR(SOCK_DHCP)) > 0)
   ...
   #ifdef _DHCP_DEBUG_   
      rt_kprintf("DHCP message : %d.%d.%d.%d(%d) %d received. \r\n",svr_addr[0],svr_addr[1],svr_addr[2], svr_addr[3],svr_port,len);
   #endif   
   }
   else return 0;
...
int8_t check_DHCP_leasedIP(void)
{
	...
	setRCR(0x03);
#ifdef _DHCP_DEBUG_
  rt_kprintf("<Check the IP Conflict %d.%d.%d.%d: ",DHCP_allocated_ip[0],DHCP_allocated_ip[1],DHCP_allocated_ip[2],DHCP_allocated_ip[3]);
  #endif
		...
		case STATE_DHCP_REQUEST :
			if (type == DHCP_ACK) 
	          ...
	          rt_kprintf("gw : %d.%d.%d.%d\r\n",DHCP_allocated_gw[0],DHCP_allocated_gw[1],DHCP_allocated_gw[2],DHCP_allocated_gw[3]);
						...
						Write_Bytes(SIPR, DHCP_allocated_ip, 4);
						Write_Bytes(SUBR, DHCP_allocated_sn, 4);
						Write_Bytes(GAR, DHCP_allocated_gw, 4);
						
						Read_Bytes(SIPR, ip_temp, 4);
						rt_kprintf("new_ip : %d.%d.%d.%d\r\n",ip_temp[0],ip_temp[1],ip_temp[2],ip_temp[3]);
						new_ip_flag = 1;
				  } 
					else 
					{
...
#ifdef _DHCP_DEBUG_
 				rt_kprintf("> Maintains the IP address \r\n");
#endif
				type = 0;
		   ...
		   ret = DHCP_IP_LEASED;
			if (type == DHCP_ACK) {
				    ...
				    OLD_allocated_ip[3] != DHCP_allocated_ip[3]) 
				{
					...
					dhcp_ip_update();
               #ifdef _DHCP_DEBUG_
                  rt_kprintf(">IP changed.\r\n");
               #endif					
				}
         #ifdef _DHCP_DEBUG_
            else rt_kprintf(">IP is continued.\r\n");
         #endif            				
				reset_DHCP_timeout();
				dhcp_state = STATE_DHCP_LEASED;
			} 
				...
				{
#ifdef _DHCP_DEBUG_
				rt_kprintf("> Receive DHCP_NACK, Failed to maintain ip\r\n");
#endif
				reset_DHCP_timeout();
				dhcp_state = STATE_DHCP_DISCOVER;
			} 
