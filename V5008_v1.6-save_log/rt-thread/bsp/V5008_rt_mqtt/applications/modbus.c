...
{	
	...
	my_memcpy(&Tem_Hum_buff[1], (char*)&UW[module].M_ID, 4, 4);
	for(i=0; i<6; i++)
	{
	...
	//===温度数据
		Tem_Hum_buff[4*i+5] = (char)((UW[module].TemperatureHumidity[2*i] &   0x00FF0000) >> 16) * PN;
		Tem_Hum_buff[4*i+6] = (char)((UW[module].TemperatureHumidity[2*i] &   0x0000FF00) >> 8);
		
	//===湿度数据==
		Tem_Hum_buff[4*i+7] = (char)((UW[module].TemperatureHumidity[2*i+1] & 0x00FF0000) >> 16);
		Tem_Hum_buff[4*i+8] = (char)((UW[module].TemperatureHumidity[2*i+1] & 0x0000FF00) >> 8);
	}
}
