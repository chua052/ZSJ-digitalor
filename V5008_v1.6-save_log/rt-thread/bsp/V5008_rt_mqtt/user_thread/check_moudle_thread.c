...
unsigned char pub_test_flag = 0;//测试用发布标志 
...
//    rt_uint32_t value = 1;
    ...
    while (1)
    {
		...
		if(Ctrl_lab_flag == 1)
		{
			...
			{
				if(rt_sem_take(&sync_sem, 500) == RT_EOK)
				{
					if(rt_sem_take(&sync_sem2, 100) == RT_EOK)
					{
...
//						SetMiniBeepValue(0);
						...
						MBFuncWriteSingleHoldingRegister(COM3,ctrl_led_data.dev_addr,
												 ...
												 ctrl_led_data.data);
						if(rt_sem_take(&sync_sem2, 50) == RT_EOK)
						{
							res = 1;
						}
					}
