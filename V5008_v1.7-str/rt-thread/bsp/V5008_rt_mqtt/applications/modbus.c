...
//	{
		for(i=0; i<MODMAX; i++)
		{
			if(rt_sem_take(&sync_sem, 30) == RT_EOK)
			{
				MBFuncReadHoldingRegister(COM3,i + 1, 0x00, 1);
			}
			else
			{
				rt_sem_release(&sync_sem); //超时释放
			}
		}
//		sem_flag = 2;
//	}
...
//		if(Uwarn_data[i] == 0x0001)//无标签有警报
	...
	lab_sta_buff[5] = sta_head.mag_state;
	lab_sta_buff[6] = sta_head.U_mount;
	lab_sta_buff[7] = sta_head.U_State_mount;
	my_memcpy(&lab_sta_buff[1], (char*)&sta_head.module_ID, 4, 4);

	for(i=0,m=0; i<sta_head.U_State_mount; i++,m++)
	{
		lab_sta_buff[8+6*i] = sta_pack[m].U_number;
		lab_sta_buff[9+6*i] = sta_pack[m].Call_State;
		lab_sta_buff[10+6*i] = sta_pack[m].Lable_Tag[0];
		lab_sta_buff[11+6*i] = sta_pack[m].Lable_Tag[1];
		lab_sta_buff[12+6*i] = sta_pack[m].Lable_Tag[2];
		lab_sta_buff[13+6*i] = sta_pack[m].Lable_Tag[3];
	}
	
}

extern unsigned char *pub_th_data;
unsigned char Tem_Hum_buff[29];
...
{	
	...
	for(i=0; i<6; i++)
	{
		if( (UW[module].TemperatureHumidity[2*i])>>24 & 0x00000001 )
		{
			PN = 1;//温度值为正数
		}
		else
	...
	//===温度数据
		Tem_Hum_buff[4*i+5] = (unsigned char)((UW[module].TemperatureHumidity[2*i] &   0x00FF0000) >> 16) * PN;
		Tem_Hum_buff[4*i+6] = (unsigned char)((UW[module].TemperatureHumidity[2*i] &   0x0000FF00) >> 8);
		
	//===湿度数据==
		Tem_Hum_buff[4*i+7] = (unsigned char)((UW[module].TemperatureHumidity[2*i+1] & 0x00FF0000) >> 16);
		Tem_Hum_buff[4*i+8] = (unsigned char)((UW[module].TemperatureHumidity[2*i+1] & 0x0000FF00) >> 8);
	}
}
...
//				lab_ena_flag = 1;
		...
		else if((ModbusSync[com].addr >= 499) && (ModbusSync[com].addr <= 522))//温湿度
		{
			if(iszero(&fram[3], fram[2]) == 0)
			{
				...
				//if(TH_compare(UW[i].TemperatureHumidity, UW[i].TemperatureHumidityBack, 6) == 1)
				{
					...
					int buff_len = 29;
					unsigned char new_buff[58] = {0};
					hex_to_str(&Tem_Hum_buff[0], &new_buff[0], buff_len);
					pub_th_data = &new_buff[0];
		...
		{
			memcpy(&UW[i].U_CardID[ModbusSync[com].addr - 64] , &fram[3], fram[2]);
...
//			if((memcmp(UW[i].U_Warn, UW[i].U_WarnBack, 54 * 2) != 0)  || (id_flag == 1))
		...
		if(com == COM1)
			rt_sem_release(&sync_sem3);
			
	}
	else if((fram[1] == 0x06) && (ModbusSync[com].fnc == 0x06))
	{
		rt_sem_release(&sync_sem2);
	}
}
...
void ModbusPoll(void)
{
	...
	for(i = 0; i < MODMAX; i++)
	{
		...
		for(j = 0; j < sizeof(HoldingReg) / sizeof(stHoldingReg); )
		{
			...
			if(rt_sem_take(&sync_sem2, 0) == RT_EOK)
			{
				if(rt_sem_take(&sync_sem, sem_wait_time) == RT_EOK)
				{
	...
	if(rt_sem_take(&sync_sem4, 0) == RT_EOK)
	{
		if(rt_sem_take(&sync_sem3, 50) == RT_EOK)
		{
...
{
	u8 count = 0;
	while(1)//实测循环一轮大概6.5s
	{
		ModbusPoll();
		IWDG_Feed();
		
		if(connet_falg_count < 3)//20多秒
		{
			connet_flag = 1;
			connet_falg_count++;
			count = 0;
